...
const generateNoise = (gridX: number, gridY: number, scale: number, seed: number): Array<Array<number>> => {
	const generateNoise = (gridX: number, gridY: number, scale: number, seed: string | number): any[][] => {
	...
	return generateNoise(gridX, gridY, scale, seed);
};
...
export class InterpolationTaskGenerator {
	...
	private generateDotDescription(graph: IGeoInterpolationGraph) {
		...
		const edgeString = measurementPoints
			.map(({ id, distance }) => `${unknownPoint.id} -- ${id} [label="${distance}"]`)
			.join(" ");
